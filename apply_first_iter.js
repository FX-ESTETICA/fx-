...
let content = fs.readFileSync("src/features/profile/components/UserDashboard.tsx", "utf8");
...
// 2. Add states
const state_block = `  const [submitError, setSubmitError] = useState("");
...
let old_if_start = content.indexOf('  if (showMerchantPortal && applicationStatus !== "success") {');
let old_if_end = content.indexOf('  // ------------------------------------------------------------------\r\n  // 常规仪表盘场景 (Normal Dashboard)');
if (old_if_end === -1) {
    old_if_end = content.indexOf('  // ------------------------------------------------------------------\n  // 常规仪表盘场景 (Normal Dashboard)');
}

const old_if_block = content.substring(old_if_start, old_if_end);

const new_if_block = \`  if (showMerchantPortal && applicationStatus !== "success") {
    return (
...
fs.writeFileSync("src/features/profile/components/UserDashboard.tsx", content, "utf8");
console.log("Restore to first iteration complete.");
