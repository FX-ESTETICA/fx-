...
export function PrivacySettings() {
  ...
  return (
    <div className={cn(
      "w-full max-w-sm rounded-3xl p-4 flex flex-col space-y-4 border transition-all",
      isLight ? "bg-black/5 border-black/10" : "bg-white/5 border-white/10"
    )}>
      <div className="flex items-center space-x-2 px-2">
        <ShieldAlert className={cn("w-4 h-4", isLight ? "text-black/50" : "text-white/50")} />
        <span className={cn("text-xs font-mono tracking-widest uppercase", isLight ? "text-black/50" : "text-white/50")}>
          隐私防御网关 (Privacy)
        </span>
      </div>
      ...
      <div className="space-y-2">
        ...
        <div className={cn("flex items-center justify-between p-3 rounded-2xl border", isLight ? "bg-white/50 border-black/5" : "bg-black/50 border-white/5")}>
          <div className="flex flex-col">
            ...
            <span className={cn("text-[10px] mt-0.5", isLight ? "text-black/40" : "text-white/40")}>
