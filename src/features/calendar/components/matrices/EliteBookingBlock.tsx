...
export const EliteBookingBlock = ({ 
  ...
  isPast = false, isCheckedOut = false,
  ...
  const getBoxShadow = () => {
    if (isPending) return '';
    if (isCheckedOut || isPast) return 'none'; // 已过时完全去除发光阴影，变成静态死线框
    ...
    <>
      ...
      <motion.div
        ...
        className={cn(
          ...
          "transition-all duration-700 ease-[cubic-bezier(0.16,1,0.3,1)]", // 平滑过渡动画
          isTiny ? "p-1 items-center justify-center" : "p-3 flex-col justify-between",
          !isHexColor && !isCheckedOut && !isPast && isActiveBgColor(accent || ''),
          !isHexColor && !isCheckedOut && isActiveBorderColor(accent || ''),
        ...
        style={{
          ...
          backdropFilter: (isCheckedOut || isPast) ? "blur(0px)" : "blur(30px)", // 已过时也彻底移除毛玻璃
        }}
      >
      {/* 内部极光背景 (Aurora Glow) */}
      {!isCheckedOut && !isPast && (
      ...
      {isTiny ? (
        ...
        <div className="flex items-center justify-center gap-2 relative z-10 w-full truncate px-2">
          ...
          {client && (
            <>
              ...
              <span className={cn("text-xs font-black tracking-wider truncate transition-opacity", isCheckedOut ? "text-white/40" : "text-white/90")}>
                {client}
              </span>
            </>
          )}
          {/* 右上角绝对定位的小圆点 (已过时则不再显示) */}
          {!isCheckedOut && !isPast && (
            <div 
              className={cn(
        ...
        <>
          ...
          <div className="flex items-center justify-between mt-1 relative z-10 w-full">
            <span className={cn("text-[10px] font-black tracking-wider truncate max-w-[60%] transition-opacity", isCheckedOut ? "text-white/40" : "text-white/90")}>
              {client}
            </span>
            {/* 右下角绝对定位的小圆点 (已过时则不再显示) */}
            {!isCheckedOut && !isPast && (
              <div 
                className={cn(
