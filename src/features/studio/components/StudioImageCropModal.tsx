...
export const StudioImageCropModal = ({ isOpen, imageSrc, onClose, onComplete }: StudioImageCropModalProps) => {
 ...
 <h2 className="text-sm tracking-[0.2em] text-white ">
 {t('txt_6d5d49')}</h2>
 </div>
 <button 
 onClick={onClose}
 disabled={isProcessing}
 className="w-8 h-8 rounded-full bg-white/5 flex items-center justify-center text-white hover:text-white hover:bg-white/10 disabled:opacity-50"
 >
 <X className="w-4 h-4" />
 </button>
 </div>
 ...
 <span className="text-xs text-white">{t('txt_05853d')}</span>
 <input
 type="range"
 value={zoom}
 min={1}
 ...
 <Button
 variant="cyan"
 className="w-full py-4 rounded-xl tracking-widest uppercase flex items-center justify-center gap-2"
