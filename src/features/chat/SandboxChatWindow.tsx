...
export default function SandboxChatWindow({ currentUserId, receiverId, roomId }: SandboxChatProps) {
 ...
 return (
 <div className="flex flex-col h-[600px] w-full max-w-md mx-auto bg-black text-white border rounded-2xl overflow-hidden relative">
 {/* 1. 顶部全息状态栏 */}
 <div className="flex items-center justify-between px-6 py-4 bg-gradient-to-r to-black border-b shrink-0">
 ...
 <span className="text-xs ">Realtime 0ms</span>
 </div>
 ...
 <div className={`max-w-[75%] rounded-2xl p-3 ${isMe ? ' border ' : ' border '}`}>
 {/* 文本内容 */}
 {msg.content && (
 <p className="text-[14px] tracking-widest break-words">{msg.content}</p>
 )}
 ...
 src={msg.image_url} 
 alt="chat_media" 
 className="relative z-10 w-full h-auto object-cover max-h-[300px] rounded-xl "
 loading="lazy"
 />
 
 {/* 降维话术：坦诚 Web 端的缓存易失性 */}
 <div className="absolute bottom-2 right-2 z-20">
 <span className="text-[11px] text-white bg-black/60 px-2 py-1 rounded-full ">
 {t('txt_b0a14d')}</span>
 </div>
 </div>
 )}
 ...
 <div className="flex items-end space-x-3 bg-black rounded-xl p-2 border ">
 
 {/* 选择图片按钮 (隐藏原生 input) */}
 <label className="cursor-pointer shrink-0 p-2 rounded-lg ">
 <input 
 type="file" 
 accept="image/*" 
 className="hidden" 
 ...
 className="w-full bg-transparent border-none focus:ring-0 text-sm resize-none h-10 py-2 px-1 placeholder-gray-600"
 onKeyDown={(e) => {
 ...
 className="shrink-0 p-2 text-white rounded-lg flex items-center justify-center"
 >
 {isSending ? (
