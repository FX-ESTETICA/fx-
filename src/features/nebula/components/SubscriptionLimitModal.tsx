...
export const SubscriptionLimitModal = ({ isOpen, onClose, currentTier, mode = 'NODE_LIMIT', onStartGracePeriod }: SubscriptionLimitModalProps) => {
 ...
 <button 
 onClick={onClose}
 className="absolute top-4 right-4 md:top-6 md:right-6 text-white hover:text-white z-10"
 >
 ...
 <h2 className="text-2xl md:text-4xl text-white tracking-widest mb-3 md:mb-4 text-center">
 {isExpiredWarning ? '您的订阅已到期' : isUpgradeIntent ? '星际算力网络' : '公司数量上限'}
 </h2>
 
 <div className="flex flex-col md:flex-row items-center gap-2 md:gap-3 text-xs md:text-sm text-white tracking-widest text-center">
 <div className="flex items-center gap-2">
 <span>您当前的订阅为</span>
 <span className="px-2 py-0.5 rounded bg-gx-gold/20 text-gx-gold border border-gx-gold/30 ">
 {currentTier}
 </span>
 </div>
 {!isExpiredWarning && (
 ...
 className="mt-6 px-4 py-1.5 rounded-full border text-[11px] tracking-widest hover:text-white "
 >
 ...
 {/* 单店版 (FREE) - 白银 */}
 <div className={cn(
 "relative z-10 flex flex-col bg-black/40 border rounded-2xl p-5 md:p-6 group",
 currentTier === 'FREE' ? " " : "border-white/10 "
 )}>
 <h3 className="text-xl tracking-widest mb-1 md:mb-2">单店</h3>
 ...
 className="text-[11px] text-white tracking-widest uppercase whitespace-nowrap"
 >
 或开启最后 15 次紧急额度
 </button>
 </div>
 ...
 className={cn(
 "w-full py-2.5 md:py-3 rounded-xl tracking-widest uppercase border text-xs md:text-sm",
 currentTier === 'FREE'
 ? " hover:text-black "
 ...
 <h3 className="text-xl tracking-widest mb-1 md:mb-2">多店</h3>
 ...
 <div className={cn(
 "relative z-10 flex flex-col bg-black border rounded-2xl p-5 md:p-6 transform group",
 currentTier === 'PRO' 
 ? " md:-translate-y-4" 
 : "border-white/10 "
 )}>
 <div className={cn(
 "absolute -top-3 left-1/2 -translate-x-1/2 text-[11px] px-3 py-1 rounded-full uppercase tracking-widest whitespace-nowrap",
 currentTier === 'PRO' ? " text-white " : " "
 )}>
 Recommended
 </div>
 <h3 className="text-xl md:text-2xl tracking-widest mb-1 md:mb-2 ">连锁</h3>
 ...
 className={cn(
 "w-full py-3 md:py-4 rounded-xl tracking-widest uppercase border text-xs md:text-sm",
 currentTier === 'PRO' 
 ? " hover:text-white " 
 ...
 <h3 className="text-xl tracking-widest text-[#FFD700] mb-1 md:mb-2 ">集团</h3>
