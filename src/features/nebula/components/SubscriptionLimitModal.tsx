    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 font-sans">
        ...
        <motion.div 
          ...
          className="relative w-full max-w-4xl bg-[#0a0a0a] border border-white/10 rounded-3xl shadow-[0_0_100px_rgba(0,0,0,0.8)] overflow-hidden"
        >
          {/* Header 锁定提示 */}
          <div className="flex flex-col items-center justify-center pt-12 pb-8 px-6 relative">
            <button 
              onClick={onClose}
            ...
            <div className="w-16 h-16 rounded-2xl bg-gx-gold/10 border border-gx-gold/20 flex items-center justify-center text-gx-gold mb-6 shadow-[0_0_30px_rgba(250,204,21,0.2)]">
            ...
            <div className="flex items-center gap-3 text-sm text-white/60 tracking-widest font-mono">
              <span>您当前的订阅为</span>
              <span className="px-2 py-0.5 rounded bg-gx-gold/20 text-gx-gold border border-gx-gold/30 font-black">
                {currentTier}
              </span>
              {!isExpiredWarning && (
                <span>支持最大公司上限：<strong className="text-white text-lg">{maxNodes}</strong>。</span>
              )}
            </div>
            
            <p className="mt-4 text-sm text-white/40 tracking-wider">
              {isExpiredWarning ? '您的全功能体验时间已耗尽，日历已进入只读模式。' : '如需扩张新的公司，请升级订阅。'}
            </p>
          </div>

          {/* Pricing Matrix 算力矩阵 */}
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 p-8 bg-white/[0.02] border-t border-white/5 relative">
            ...
            {/* 单店版 (FREE) - 白银 */}
            <div className={cn(
              "relative z-10 flex flex-col bg-black/40 border rounded-2xl p-6 transition-all group",
              ...
              <h3 className="text-xl font-black tracking-widest text-gray-300 mb-2">单店</h3>
              <p className="text-xs text-gray-400 tracking-wider mb-8">支持单个商店</p>
                ...
                {isExpiredWarning && currentTier === 'FREE' && onStartGracePeriod && (
                  <div className="absolute -top-10 left-1/2 -translate-x-1/2 w-full text-center">
                    <button 
                      ...
                      className="text-[10px] text-white/30 hover:text-gray-300 transition-colors font-mono tracking-widest uppercase"
                    >
                      或开启最后 15 次紧急额度
                    </button>
                  </div>
            ...
            <div className={cn(
              ...
              <h3 className="text-xl font-black tracking-widest text-gx-cyan mb-2">多店</h3>
              <p className="text-xs text-gx-cyan/60 tracking-wider mb-8">支持 3 家商店</p>
            ...
            <div className={cn(
              "relative z-10 flex flex-col bg-black border rounded-2xl p-6 transform transition-all group",
              currentTier === 'PRO' 
              ...
              <div className={cn(
                "absolute -top-3 left-1/2 -translate-x-1/2 text-[10px] font-black px-3 py-1 rounded-full uppercase tracking-widest transition-all",
                currentTier === 'PRO' ? "bg-gx-purple text-white shadow-[0_0_10px_rgba(188,19,254,0.5)]" : "bg-gx-purple/20 text-gx-purple/80"
              )}>
                Recommended
              </div>
              <h3 className="text-2xl font-black tracking-widest text-gx-purple mb-2 drop-shadow-[0_0_10px_rgba(188,19,254,0.5)]">连锁</h3>
              <p className="text-xs text-gx-purple/80 tracking-wider mb-8">支持 10 家商店</p>
            ...
            <div className={cn(
              ...
              <h3 className="text-xl font-black tracking-widest text-[#FFD700] mb-2 drop-shadow-[0_0_10px_rgba(255,215,0,0.3)]">集团</h3>
              <p className="text-xs text-[#FFD700]/60 tracking-wider mb-8">无上限</p>
