...
export const BookingForm = ({ initialDetails, onSubmit, onCancel }: BookingFormProps) => {
 ...
 <h2 className="text-2xl tracking-tight">{t('txt_432960')}</h2>
 ...
 <label className="text-[11px] text-white uppercase tracking-widest flex items-center gap-2">
 ...
 <div className="px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-sm ">
 ...
 className={cn(
 "w-full bg-white/5 border border-white/10 rounded-xl py-3 pl-11 pr-4 text-sm focus:outline-none ",
 errors.customerName && "border-red-500/50"
 )}
 placeholder={t('txt_0ec7d3')}
 />
 </div>
 {errors.customerName && <p className={cn("text-[11px] mt-1", isLight ? "text-black/60" : "text-white/60")}>{errors.customerName.message}</p>}
 </div>
 ...
 <div className="relative group">
 <Phone className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-white " />
 <input
 ...
 {errors.customerPhone && <p className={cn("text-[11px] mt-1", isLight ? "text-black/60" : "text-white/60")}>{errors.customerPhone.message}</p>}
 </div>
 </div>
 ...
 <textarea
 {...register("notes")}
 className="w-full bg-white/5 border border-white/10 rounded-xl py-3 px-4 text-sm focus:outline-none min-h-[100px] resize-none"
