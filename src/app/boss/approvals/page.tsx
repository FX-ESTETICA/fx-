 ...
 *,
 ...
 <div>
 <h1 className="text-2xl tracking-tighter text-red-500 flex items-center gap-3">
 <ShieldAlert className="w-6 h-6" />
 {t('txt_f79337')}</h1>
 <p className="text-[11px] tracking-[0.2em] text-white mt-1 uppercase">Ascension Command Center // NEXUS</p>
 </div>
 </div>
 </header>

 {isLoading ? (
 <div className="flex justify-center py-20 text-white text-xs ">
 SYNCING WITH MATRIX...
 </div>
 ...
 <p className="text-sm text-white tracking-widest">{t('txt_772970')}</p>
 </div>
 ) : (
 ...
 key={app.id} 
 className={cn(
 "p-6 flex flex-col md:flex-row md:items-center justify-between gap-6 ",
 ...
 <h3 className="text-xl tracking-tight">{app.brand_name}</h3>
 <span className={cn(
 "px-2 py-0.5 rounded text-[11px] tracking-widest uppercase border",
 ...
 {app.nexus_code && (
 <p className="text-xs text-gx-gold">
 ...
 className="px-4 py-2 rounded-lg bg-black border border-white/10 text-white hover:text-white hover:bg-white/5 text-xs tracking-widest uppercase disabled:opacity-50"
 >
 {t('txt_325254')}</button>
 <button 
 onClick={() => handleApprove(app)}
 disabled={processingId === app.id}
 className="px-6 py-2 rounded-lg bg-red-500/20 border border-red-500/50 text-red-500 hover:bg-red-500 hover:text-white text-xs tracking-widest uppercase disabled:opacity-50 flex items-center gap-2"
 >
 {processingId === app.id ? <Clock className="w-4 h-4 animate-spin" /> : <CheckCircle2 className="w-4 h-4" />}
 {t('txt_b20cc6')}</button>
 </>
 ) : (
 <div className="px-4 py-2 rounded text-xs tracking-widest text-white border border-white/5 bg-black/50">
