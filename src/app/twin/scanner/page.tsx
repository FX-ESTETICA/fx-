    ...
    if (this.state.hasError) {
      return (
        <div className="absolute inset-0 flex flex-col items-center justify-center bg-red-950/80 z-50 p-4 text-center">
          ...
          <h1 className="text-xl font-bold text-red-400 mb-2">3D Render Error</h1>
          <p className="text-sm text-red-200 font-mono whitespace-pre-wrap">{this.state.error?.message}</p>
...
export default function ScannerSandboxPage() {
  ...
  return (
    <div className="relative w-full h-screen bg-[#050505] overflow-hidden text-white font-sans flex flex-col md:flex-row">
      ...
      <div className="relative w-full md:w-1/2 h-1/2 md:h-full border-b md:border-b-0 md:border-r  flex items-center justify-center overflow-hidden bg-black transition-all duration-1000">
        {scanPhase !== 'rendered' ? (
          <>
            <video 
              ref={videoRef}
              ...
              <div className="absolute inset-0 z-20 pointer-events-none flex flex-col items-center justify-center">
                ...
                {scanPhase === 'scanning' && (
                  <>
                    ...
                    <div className="absolute top-8 left-1/2 -translate-x-1/2 flex items-center gap-2 bg-black/50  px-4 py-1.5 rounded-full border ">
                      ...
                      <span className="text-[10px]  uppercase tracking-widest">LUX: OPTIMAL (450)</span>
                    </div>
                ...
                {(scanPhase === 'processing' || scanPhase === 'complete') && (
                  <div className="absolute inset-0   flex flex-col items-center justify-center gap-6 z-30">
                    ...
                    <h2 className="text-2xl font-mono  tracking-widest uppercase text-center">
                ...
                <div className="absolute top-24 left-6 right-6 flex justify-between items-start">
                  ...
                  <div className={`flex flex-col gap-4 pointer-events-auto bg-black/40  p-4 rounded-xl border  w-48  transition-opacity ${scanPhase !== 'idle' ? ' pointer-events-none' : 'opacity-100'}`}>
                    ...
                    <div className="flex flex-col gap-3">
                      <label className="flex flex-col gap-1">
                        <span className="text-[10px]  uppercase tracking-widest font-mono">Height (cm)</span>
                        <input 
                          type="number" 
                          value={inputHeight}
                          onChange={(e) => setInputHeight(Number(e.target.value))}
                          className="bg-black/50 border   text-lg font-mono p-2 rounded focus:outline-none  transition-colors"
                        />
                      </label>
                      
                      <label className="flex flex-col gap-1">
                        <span className="text-[10px]  uppercase tracking-widest font-mono">Weight (kg)</span>
                        <input 
                          type="number" 
                          value={inputWeight}
                  ...
                  <div className="flex flex-col items-end gap-3">
                    <div className="flex flex-col items-end">
                      <span className="text-[10px]  uppercase tracking-widest font-mono mb-1">Generated BMI Core</span>
                    ...
                    <button 
                      ...
                      className="pointer-events-auto mt-2 px-6 py-3  border   text-xs uppercase tracking-[0.2em] font-bold rounded-full  hover:scale-105 transition-all active:scale-95  disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
                    >
                      {scanPhase !== 'idle' ? (
                        <>
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center z-30">
            <h2 className="text-2xl font-mono  tracking-widest uppercase mb-4 animate-pulse">
              Compute Transferred
            </h2>
            <p className="text-xs  uppercase tracking-[0.2em] font-mono text-center">
              Camera offline.<br/>
              Memory freed.<br/>
        ...
        {capturedImages.length === 0 && scanPhase === 'idle' ? (
          <div className="flex flex-col items-center justify-center gap-6 max-w-sm text-center z-20">
            ...
            <button 
              ...
              className="mt-8 px-6 py-2.5 border   text-xs uppercase tracking-[0.2em] rounded-full   transition-all duration-300"
            >
              Enter Sandbox
            </button>
          </div>
          ...
          <div className="flex flex-col items-center justify-center gap-6 w-full max-w-2xl text-center z-20 px-8">
            <div className={`flex flex-col gap-2 w-full transition-opacity duration-1000 ${scanPhase === 'rendered' ? 'opacity-0 h-0 overflow-hidden' : 'opacity-100'}`}>
              <div className="flex justify-between items-end border-b  pb-2 mb-4">
                <div className="flex flex-col">
                  <h2 className="text-xl font-mono  tracking-widest uppercase">3DGS Video Capture</h2>
                  <span className="text-xs  mt-1 uppercase">Continuous spatial recording active</span>
                </div>
                <div className="flex flex-col items-end gap-1">
                  <span className="text-xs  font-mono">{Math.round(recordingProgress)}%</span>
              ...
              <div className="grid grid-cols-4 gap-4 w-full">
                {[0, 45, 90, 135, 180, 225, 270, 315].map((angle) => {
                  ...
                  return (
                    <div 
                      key={angle} 
                      className={`relative aspect-[3/4] border ${isCaptured ? ' ' : isCapturing ? ' ' : ''} rounded-lg overflow-hidden bg-black/50  transition-all duration-500`}
                    >
                      {isCaptured ? (
                        <div className="absolute inset-0 flex items-center justify-center ">
                      ...
                      <div className="absolute bottom-0 left-0 w-full bg-gradient-to-t from-black to-transparent p-2 text-left">
                        <span className="text-[10px] font-mono  font-bold ">{angle}° MAPPED</span>
                      </div>
                      {isCaptured && (
              ...
              {scanPhase === 'processing' && (
                <div className="mt-8 flex flex-col items-center gap-4 animate-pulse">
                  ...
                  <div className="text-center">
                    <h3 className=" text-sm font-mono uppercase tracking-widest">Transmitting to GPU Cluster</h3>
            ...
            {scanPhase === 'rendered' ? (
              <div className="w-full h-[60vh] md:h-[80vh] relative animate-in fade-in zoom-in duration-1000">
                ...
                <div className="absolute bottom-4 left-1/2 -translate-x-1/2 z-50">
                  <button 
                    ...
                    className="px-8 py-3  text-black text-sm uppercase tracking-[0.2em] font-bold rounded-full  hover:scale-105 transition-all "
                  >
                    Enter Sandbox
                  </button>
                </div>
              ...
              <button 
                ...
                className="mt-8 px-8 py-3  border   text-sm uppercase tracking-[0.2em] font-bold rounded-full  hover:scale-105 transition-all active:scale-95 "
              >
