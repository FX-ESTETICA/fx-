...
export default function VisionPage() {
 ...
 {/* 由于是在 /vision 独立路由，底层 .tsx 的 NebulaBackground 会自动作为最底层星空背景 */}
 ...
 <span className="text-3xl md:text-4xl tracking-tighter text-transparent bg-clip-text bg-gradient-to-br from-white ">
 GX
 </span>
 <span className="text-lg md:text-xl tracking-tighter transform -translate-y-3 md:-translate-y-4 -translate-x-1.5 md:-translate-x-2 ">
 +
 </span>
 <span className="text-xl md:text-2xl tracking-widest text-transparent bg-clip-text bg-gradient-to-r ">
 {t('txt_b05e70')}</span>
 </div>
 </div>
 </div>
 ...
 activeFeature === idx ? "opacity-100 xl:scale-100" : "xl: xl:scale-95"
 )}
 >
 ...
 <h3 className="text-2xl lg:text-3xl text-white">{feat.title}</h3>
 </div>
 ...
 className="hidden md:flex group relative z-50 items-center justify-center gap-3 w-fit px-10 py-5 rounded-full bg-white text-black text-lg overflow-hidden "
 >
 <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/40 to-transparent translate-x-[-100%] group-hover:translate-x-[100%] " />
 ...
 <h3 className="text-lg text-white">{feat.title}</h3>
 </div>
 ...
 onClick={handleEnterSandbox}
 className="mt-4 group relative flex items-center justify-center gap-3 w-full py-5 rounded-2xl bg-white text-black text-lg overflow-hidden "
 >
 <span>{t('txt_86ef01')}</span>
 <ChevronRight className="w-5 h-5 group-hover:translate-x-1 " />
 </motion.button>
 ...
 className={cn(
 "absolute inset-0 w-full h-full object-cover -out", 
