...
export default function ExplorePage() {
  ...
  useEffect(() => {
    async function fetchPosts() {
      ...
      if (error) {
        ...
        const formattedPosts = data.map(post => ({
          id: post.id,
          user: {
            name: post.profiles?.full_name || 'GX⁺ 用户',
            avatar: post.profiles?.avatar_url || '/wallhaven-qr3o8q.png',
          },
    ...
    fetchPosts()
  }, [refreshKey]) // 监听 refreshKey

  // 组合模拟数据和数据库数据
  const allPosts = [...dbPosts, ...EXPLORE_POSTS]
