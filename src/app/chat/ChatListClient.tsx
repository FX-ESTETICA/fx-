...
export default function ChatListPage() {
 ...
 // 【世界顶端：身份水合锁 (The Identity Hydration Lock)】
 // 如果身份还没有从 LocalStorage / Session 中解析完毕，坚决不渲染 ChatListUI。
 // 宁愿呈现 0.1 秒的绝对透明，也绝不允许携带错误的默认身份去请求数据库，从而引发幽灵闪烁。
 if (!isRoleLoaded) {
 return (
 <main className="relative w-full h-[100dvh] bg-transparent overflow-hidden flex items-center justify-center">
 {/* 极简降维占位：连 Loading 都不要，保持纯净 */}
 </main>
 );
 }
 ...
 // 终极修复：移除私有 bg-black，让全局 .tsx 中的 NebulaBackground 完美透射上来，彻底消灭“双重宇宙”导致的 WebGL 闪烁灾难。
 ...
 */}
 <div 
 className={`absolute inset-0 w-full h-full pointer-events-none ${activeChat ? 'opacity-100 z-0' : 'opacity-0 -z-10'} ${isLight ? 'bg-white/50' : 'bg-black/50'}`}
 />
