...
export const BottomNavBar = ({ className }: { className?: string }) => {
 ...
 // 主舞台之外的页面点击底导时，先回到对应入口，再同步 Tab 状态。
 if (!["/", "/home", "/dashboard", "/me", "/calendar"].some(p => pathname === p || pathname.startsWith("/calendar/"))) {
 window.location.href = id === 'calendar' ? '/calendar' : `/${id}`;
