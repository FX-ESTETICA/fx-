...
export function GxProCard({ shop, onClick, index = 0 }: GxProCardProps) {
 ...
 onClick={onClick}
 className="cursor-pointer relative w-full aspect-[16/9] rounded-3xl overflow-hidden group border border-yellow-500/20 hover:border-yellow-500/50 shrink-0"
 >
 {/* 边缘流光边框效果 */}
 ...
 <span className="text-[11px] text-yellow-500 tracking-widest uppercase ">{t('txt_53a12c')}</span>
 </div>
 ...
 <h3 className="text-2xl sm:text-3xl tracking-tighter uppercase text-white line-clamp-1">
 {shop.name}
 </h3>
 <p className="text-xs text-white tracking-widest line-clamp-1 uppercase">
 {slogan}
 </p>
 </div>
 
 {/* 右下角：立即预约按钮 */}
 <div className="shrink-0 flex items-center gap-1.5 text-yellow-400 text-xs bg-yellow-500/10 px-4 py-2 rounded-full border border-yellow-500/30 pointer-events-auto">
