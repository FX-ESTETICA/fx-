...
export interface ShopDetailViewProps {
 ...
 hideContent?: boolean; // 新增：是否隐藏除顶部大图外的所有内容
}
...
export function ShopDetailView({
 ...
 return (
 <div className={cn("w-full bg-[#0a0a0a] min-h-[100dvh] ", !isCompact && "md:pb-24", hideContent && "bg-black pb-0 md:pb-0")}>
 {/* Detail View Hero Image */}
 <div className={cn(
 "w-full bg-white/5 relative ", 
 isCompact 
 ? (hideContent ? "h-[100dvh]" : "aspect-[4/3]") 
 : (hideContent ? "h-[100dvh]" : "h-[40vh] md:h-[60vh]")
 )}>
 ...
 <ImagePlus className="w-6 h-6 text-white mb-2" />
 <span className="text-white text-[11px] tracking-widest">{t('txt_36b5f5')}</span>
 </div>
 )}
 ...
 <span className="text-white text-[11px] tracking-widest">{t('txt_b59359')}</span>
 </div>
 )}
 {/* 画廊顶部光影遮罩 */}
 ...
 {/* SWIPE Badge */}
 <div className="absolute top-6 left-6 px-3 py-1.5 rounded-lg bg-black/40 border border-white/10 text-[11px] tracking-widest text-white uppercase flex items-center gap-2">
 ...
 {coverImages.map((_, i) => (
 <div key={i} className={cn("h-1 rounded-full ", i === 0 ? "w-4 bg-white" : "w-1.5 bg-white/30")} />
 ))}
 </div>
 )}
 </div>
 ...
 <div className="absolute top-0 right-6 -translate-y-1/2 translate-x-1/2 z-20 flex flex-col items-center justify-center w-12 h-12 rounded-full bg-gradient-to-br from-[#2a2a2a] via-black to-[#111] border border-gx-gold/30 cursor-pointer group">
 <Star className="w-3.5 h-3.5 fill-gx-gold text-gx-gold mb-0.5" />
 <span className="text-gx-gold text-[11px] leading-none">5.0</span>
 <div className="absolute inset-0 rounded-full bg-gx-gold/0 group-hover:bg-gx-gold/10 " />
 </div>

 <div className="flex justify-between items-start mb-2 pr-6 pt-2">
 <h1 className="text-2xl leading-tight text-white truncate w-full">{storeName || "公司名称"}</h1>
 </div>
 ...
 <div className="flex items-start justify-between text-xs text-white mb-3">
 <div className="flex items-center gap-2">
 <Clock className="w-3.5 h-3.5 shrink-0" />
 <span className="whitespace-nowrap">{t('txt_cc3307')}</span>
 {hours && hours.length > 0 && (
 <span className=" whitespace-nowrap text-white ml-2">
 {String(hours[0].start).padStart(2, '0')}:00 - {String(hours[0].end).padStart(2, '0')}:00
 </span>
 )}
 </div>
 <div className={cn(
 "px-2 py-0.5 rounded text-[11px] tracking-wider shrink-0 ml-2",
 storeStatus === 'open' ? " " :
 ...
 <div 
 className="flex items-start gap-2 text-xs text-white cursor-pointer group"
 ...
 className="w-full mt-5 py-3 rounded-xl flex items-center justify-center gap-2 text-xs tracking-widest border "
 >
 ...
 <h3 className="text-[11px] text-white uppercase tracking-widest mb-4 flex items-center gap-2">
 ...
 className="bg-white/5 border border-white/10 p-4 rounded-2xl flex flex-col justify-between group cursor-pointer relative overflow-hidden h-full min-h-[100px]"
 >
 <div className="absolute top-0 bottom-0 left-0 w-1 " />
 <div className="flex justify-between items-start mb-4">
 <div>
 <h4 className="text-sm text-white line-clamp-2 pr-2">{cap.label || cap.name}</h4>
 <div className="text-[11px] text-white mt-1.5 flex items-center gap-1.5">
 <Clock className="w-3 h-3" /> {cap.duration || "60"} MIN
 </div>
 </div>
 </div>
 <div className="flex justify-between items-end mt-auto">
 <div className="text-[11px] border px-2 py-0.5 rounded-full">{t('txt_26ad89')}</div>
 <div className="text-lg text-white ">{cap.price}</div>
 </div>
 </div>
 ))}
 </div>
 ) : (
 <div className="w-full py-10 border border-dashed border-white/10 rounded-2xl flex flex-col items-center justify-center text-white bg-white/5">
 <Plus className="w-6 h-6 mb-2 " />
 <span className="text-[11px] tracking-widest">{t('txt_637f84')}</span>
 </div>
 )}
 </div>
 </div>
